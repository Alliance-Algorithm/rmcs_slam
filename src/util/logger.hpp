...
#define RMCS_MAKE_LOGGER_LAMBDA(NAME)                                                              \
    const auto rclcpp_info = [](auto&&... args) {                                                  \
        RCLCPP_INFO(rclcpp::get_logger(NAME), std::forward<decltype(args)>(args)...);              \
    };                                                                                             \
    const auto rclcpp_warn = [](auto&&... args) {                                                  \
        RCLCPP_WARN(rclcpp::get_logger(NAME), std::forward<decltype(args)>(args)...);              \
    };                                                                                             \
    const auto rclcpp_error = [](auto&&... args) {                                                 \
        RCLCPP_ERROR(rclcpp::get_logger(NAME), std::forward<decltype(args)>(args)...);             \
    };
...
inline auto runtime_error(const std::string& text) {
    return std::runtime_error { error_text(text) };
}
