...
public:
    ...
    std::pair<Eigen::Isometry3f, PointCloudT> query(
        const std::shared_ptr<PointCloudT>& pointcloud) {
        const auto mat = LidarIris::GetIris(*pointcloud);
        const auto feature = descriptor_core.GetFeature(mat);
        ...
        rclcpp_info("query result: bias[%d] distance[%4.2f] x[%4.2f] y[%4.2f] key size[%zu]",
            bias_min, distance_min, point_min.x(), point_min.y(), frame_min.size());
...
private:
    ...
    void generate_key_frames(const std::shared_ptr<PointCloudT>& pointcloud) {
        ...
        for (const auto& point : key_points) {
            ...
            auto transform = Eigen::Affine3f { Eigen::Translation3f { -point.x(), -point.y(), 0 }
...
std::pair<Eigen::Isometry3f, DescriptorManager::PointCloudT> DescriptorManager::query(
    const std::shared_ptr<PointCloudT>& pointcloud) {
