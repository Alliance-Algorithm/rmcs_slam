...
void SLAM::signal_handle(int signal)
{
...
void SLAM::dump_lio_state_to_log(FILE* file)
{
    ...
    fprintf(file, "%lf %lf %lf ", state_point_.pos(0), state_point_.pos(1), state_point_.pos(2));    // Pos
    fprintf(file, "%lf %lf %lf ", 0.0, 0.0, 0.0);                                                    // omega
    fprintf(file, "%lf %lf %lf ", state_point_.vel(0), state_point_.vel(1), state_point_.vel(2));    // Vel
    fprintf(file, "%lf %lf %lf ", 0.0, 0.0, 0.0);                                                    // Acc
    fprintf(file, "%lf %lf %lf ", state_point_.bg(0), state_point_.bg(1), state_point_.bg(2));       // Bias_g
    fprintf(file, "%lf %lf %lf ", state_point_.ba(0), state_point_.ba(1), state_point_.ba(2));       // Bias_a
    fprintf(file, "%lf %lf %lf ", state_point_.grav[0], state_point_.grav[1], state_point_.grav[2]); // Bias_a
    fprintf(file, "\r\n");
    fflush(file);
}

void SLAM::point_body_to_world_ikfom(PointType const* const in, PointType* const out, state_ikfom& state)
{
