...
public:
    ...
    void process(const std::unique_ptr<LivoxMsg>& msg, PointCloudXYZI::Ptr& dst);

    void process(const sensor_msgs::msg::PointCloud2::UniquePtr& msg, PointCloudXYZI::Ptr& pcl_out);
    void set(bool feat_en, int lid_type, double bld, int pfilt_num);

    PointCloudXYZI pointcloud_full, pointcloud_corn, pointcloud_surf;

    PointCloudXYZI pointcloud_buff[128]; // maximum 128 line lidar
...
private:
    ...
    int plane_judge(
        const PointCloudXYZI& pl, std::vector<orgtype>& types, uint i, uint& i_nex,
