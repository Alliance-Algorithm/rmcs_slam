...
void SLAM::lidar_a_subscription_callback(const livox_ros_driver2::msg::CustomMsg::UniquePtr& msg) {
    global_mutex.lock();
    ...
    preprocess_->process(msg, pointcloud);
    lidar_buffer_.push_back(pointcloud);
...
void SLAM::lidar_b_subscription_callback(const livox_ros_driver2::msg::CustomMsg::UniquePtr& msg) {}
...
void SLAM::imu_subscription_callback(const sensor_msgs::msg::Imu::UniquePtr& msg_in) {
    ...
    if (std::abs(time_diff_lidar_wrt_imu_) > 0.1 && time_sync_) {
        msg->header.stamp = rclcpp::Time( //
            int64_t(time_diff_lidar_wrt_imu_ + get_time_sec(msg_in->header.stamp)));
    }
...
void SLAM::publish_effect_world(
    const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& publisher) {
