...
//
//-----------------------------------------------------------------------------------------------------
void highpass(const Size& sz, Mat& dst) {
...
float logpolar(Mat& src, Mat& dst) {
    float radii  = float(src.cols);
    float angles = float(src.rows);
    Point2f center(float(src.cols) / 2.f, float(src.rows) / 2.f);
    float d        = float(cv::norm(Vec2f(float(src.cols) - center.x, float(src.rows) - center.y)));
    float log_base = std::pow<float>(10.0, log10(d) / radii);
    float d_theta  = CV_PI / (float)angles;
    float theta    = CV_PI / 2.0;
    float radius   = 0;
    Mat map_x(src.size(), CV_32FC1);
    Mat map_y(src.size(), CV_32FC1);
    for (int i = 0; float(i) < angles; ++i) {
        for (int j = 0; float(j) < radii; ++j) {
...
//-----------------------------------------------------------------------------------------------------
RotatedRect LogPolarFFTTemplateMatch(
    Mat& im0, Mat& im1 /*, double canny_threshold1, double canny_threshold2*/) {
    // Accept 1 or 3 channel CV_8U, CV_32F or CV_64F images.
    CV_Assert(
        (im0.type() == CV_8UC1) || (im0.type() == CV_8UC3) || (im0.type() == CV_32FC1)
        || (im0.type() == CV_32FC3) || (im0.type() == CV_64FC1) || (im0.type() == CV_64FC3));
    ...
    auto angle = 180.0 * rotation_and_scale.y / f0lp.rows;
    auto scale = std::pow(log_base, rotation_and_scale.x);
    // --------------
    if (scale > 1.8) {
        rotation_and_scale = cv::phaseCorrelate(f1lp, f0lp);
        angle              = -180.0f * static_cast<float>(rotation_and_scale.y / f0lp.rows);
        scale              = 1.0f / std::pow(log_base, static_cast<float>(rotation_and_scale.x));
        if (scale > 1.8) {
    ...
    RotatedRect rr;
    rr.center = tr + Point2d(static_cast<double>(im0.cols) / 2, static_cast<double>(im0.rows) / 2);
    rr.angle  = static_cast<float>(-angle);
    rr.size.width  = static_cast<float>(im1.cols / scale);
    rr.size.height = static_cast<float>(im1.rows / scale);
